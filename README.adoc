:jarfilename: solr-hive-serde-<version>.jar
:jarfilenamehive013: solr-hive-serde-0.13-<version>.jar
...
* `solr-hive-serde/build/libs/{jarfilename}`
* `solr-hive-serde-0.13/build/libs/{jarfilenamehive013}`
...
<3> Add the `{jarfilename}` file to Hive. Note the jar name shown here omits the version information which will be included in the jar file you have. If you are using Hive 0.13, you must also use a jar specifically built for 0.13.
...
<5> Define the custom storage handler provided by the `{jarfilename}`.
