...
This will make two .jar files, one for use with Hive v0.13 *only*, and another that can be used with Hive 0.12, 0.14, and 0.15.

[listing]
solr-hive-serde/build/libs/solr-hive-serde-{version}.jar
solr-hive-serde-0.13/build/libs/solr-hive-serde-0.13-{version}.jar
...
<3> Add the `solr-hive-serde.jar` file to Hive. Note the jar name shown here omits the version information which will be included in the jar file you have. If you are using Hive 0.13, you must also use a jar specifically built for 0.13.
...
<5> Define the custom storage handler provided by the `solr-hive-serde.jar`.
<6> Define storage location in HDFS.
<7> Define the location of Solr (or ZooKeeper if using SolrCloud), the collection in Solr to index the data to, and the query to use when reading the table.
// end::example[]
